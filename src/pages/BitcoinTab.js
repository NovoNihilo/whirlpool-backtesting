...
function BitcoinTab() {
  ...
  const strategies = [
    ...
    { key: 'strategy3', name: '50% BTC (0.3% APY) / 50% USDC (7% APY) + (Daily Rebalance)' }
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/data/bitcoin.csv');
        const text = await response.text();
        const data = parseCSV(text);
        setPriceData(data);
      } catch (err) {
        console.error('Error fetching Bitcoin data:', err);
        setError('Failed to load Bitcoin price data. Please ensure the CSV file is in the correct location.');
      }
    };
  ...
  const handleVisualize = () => {
    ...
    try {
      ...
      const strategy2Data = calculate5050Performance(filtered, investment, 0.3, 7); // Bitcoin gets 0.3% APY, USDC gets 7% APY
      const strategy3Data = calculateRebalancedPerformance(filtered, investment, 0.3, 7);
  ...
  return (
    <div className="tab-content">
      <h2>Bitcoin Basket Performance</h2>
