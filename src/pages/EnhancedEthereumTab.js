...
function EnhancedEthereumTab() {
  ...
  const strategies = [
    { key: 'strategy1', name: '100% ETH' },
    { key: 'strategy2', name: '50% ETH (3.5% APY) / 50% USDC (7% APY)' },
    { key: 'strategy3', name: `50% ETH (3.5% APY) / 50% USDC (7% APY) + (${rebalanceFrequency > 1 ? rebalanceFrequency + 'x Daily' : 'Daily'} Rebalance)` }
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/ethereum_1hour_complete.csv');
        const text = await response.text();
        const data = parseHourlyCSV(text);
        setHourlyData(data);
      } catch (err) {
        console.error('Error fetching hourly Ethereum data:', err);
        setError('Failed to load hourly Ethereum price data. Please ensure the CSV file is in the correct location.');
      }
    };
  ...
  useEffect(() => {
    strategies[2].name = `50% ETH (3.5% APY) / 50% USDC (7% APY) + (${rebalanceFrequency > 1 ? rebalanceFrequency + 'x Daily' : 'Daily'} Rebalance)`;
  }, [rebalanceFrequency]);
  ...
  const handleVisualize = () => {
    ...
    try {
      ...
      const strategy2Data = calculate5050PerformanceHourly(filtered, investment, 3.5, 7); // ETH gets 3.5% APY, USDC gets 7% APY
      const strategy3Data = calculateRebalancedPerformanceHourly(filtered, investment, 3.5, 7, rebalanceFrequency);
  ...
  return (
    <div className="tab-content">
      <h2>Ethereum Basket Performance (Hourly Data)</h2>
...
export default EnhancedEthereumTab;