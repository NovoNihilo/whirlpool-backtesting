...
function EthereumTab() {
  ...
  const strategies = [
    ...
    { key: 'strategy3', name: '50% ETH (3.5% APY) / 50% USDC (7% APY) + (Daily Rebalance)' }
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/data/ethereum.csv');
        const text = await response.text();
        const data = parseCSV(text);
        setPriceData(data);
      } catch (err) {
        console.error('Error fetching Ethereum data:', err);
        setError('Failed to load Ethereum price data. Please ensure the CSV file is in the correct location.');
      }
    };
  ...
  const handleVisualize = () => {
    ...
    try {
      ...
      const strategy2Data = calculate5050Performance(filtered, investment, 3.5, 7); // ETH gets 3.5% APY, USDC gets 7% APY
      const strategy3Data = calculateRebalancedPerformance(filtered, investment, 3.5, 7);
  ...
  return (
    <div className="tab-content">
      <h2>Ethereum Basket Performance</h2>
